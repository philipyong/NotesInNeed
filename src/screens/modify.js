...
export default function ModifyScreen({ route, navigation, extraData }) {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", () => {
      const { id, title, tags, body } = route.params;
      setId(id);
      setTitle(title);
      setTags(tags);
      setBody(body);
  ...
  function onAddButtonPress() {
    if (title === "") {
      ...
      db.collection("users").doc(extraData.id).collection("notes").doc(id).set({
        title: title,
        tags: tags,
        body: body,
    ...
    <Home>
      <Header>
        <Header.Group>
          ...
          <Header.Title>Edit Note</Header.Title>
        </Header.Group>
        <Header.Group>
